...
def load_nhanes() -> pd.DataFrame:
    ...
    mortality_months = pd.to_numeric(df["mortality_months"], errors="coerce")
    all_cause_death = df["all_cause_death"].astype(bool)
    death_within_1y = (all_cause_death & mortality_months.le(12)).astype(int)
    known_1y = mortality_months.ge(12) | death_within_1y.eq(1)
    out = pd.DataFrame(
        {
            "source": "NHANES",
            ...
            "outcome": death_within_1y,
            "outcome_name": "death_within_1y",
            "known_1y": known_1y,
        }
    )
    out["known_outcome"] = out["known_1y"].astype(bool)
    ...
    out = pd.DataFrame(
        {
            ...
            "outcome": pd.to_numeric(df["death_within_1y_discharge"], errors="coerce"),
            "outcome_name": "death_within_1y_discharge",
        }
    )
...
def main() -> None:
    ...
    metadata = {
        "purpose": "Common-feature transportability stress test using 1-year mortality endpoints where available. NHANES uses death within 12 months; MIMIC-IV uses in-hospital death or post-discharge death within 365 days.",

...
This sensitivity analysis repeats the three primary transported logistic regressions without class weighting, using the same cohorts, deterministic splits, feature sets, and preprocessing as the primary class-weighted analysis. ECE is reported using both 10 equal-frequency bins and 10 equal-width bins.
...
Interpretation: class weighting had little effect on discrimination but materially changed absolute probability levels and ECE. This supports treating the primary class-weighted probability levels cautiously and using slope/intercept diagnostics plus unweighted sensitivity analyses when the model is intended for risk estimation.
